...
Single dispatcher in [src/index.ts](../src/index.ts) serializes per thread: two messages from the same chat never run two agent calls in parallel. The outbox is the only path for asynchronous side-channel messages — the agent's MCP tools enqueue rows, the drain loop delivers them.

## Components

| Layer                       | Where                                                     | Notes |
|---|---|---|
| Process root                | [src/index.ts](../src/index.ts)                           | Boots channels, drain loop, health server, backup schedule. |
| Per-turn handler            | [src/agent.ts](../src/agent.ts)                           | Picks provider, runs it, catches errors, manages typing. |
| Channel router              | [src/channels/router.ts](../src/channels/router.ts)       | Dispatches by thread-id prefix: `telegram:`, `slack:`, `whatsapp:`. |
| Channel adapters            | [src/channels/](../src/channels/)                         | One file per channel. Implements the `Channel` interface. |
| Provider registry           | [src/providers/registry.ts](../src/providers/registry.ts) | Selects Claude or Gemini from `agent_provider`. |
| Claude path                 | [src/providers/claude-sdk.ts](../src/providers/claude-sdk.ts) | Long-lived `query()` per thread via `@anthropic-ai/claude-agent-sdk`. LRU cap. |
| Gemini path                 | [src/providers/gemini-sdk.ts](../src/providers/gemini-sdk.ts) | In-process inference via `@google/gemini-cli-core`. |
| MCP server                  | [src/mcp/server.ts](../src/mcp/server.ts)                 | stdio MCP exposing channel + Google tools to the agent. |
| Per-call tool gate          | [src/lib/tool-permissions.ts](../src/lib/tool-permissions.ts) | `allowed_paths` enforcement + Bash denylist. |
| SQLite                      | [src/db/](../src/db/)                                     | `messages`, `outbox`, `sessions` tables. |
| Config                      | [src/lib/config.ts](../src/lib/config.ts)                 | `data/config.json` + env overlay, read once. |
...
`data/marsclaw.db` (override with `MARSCLAW_DB`). Migrations live in [migrations/](../migrations/) and run on every boot.
...
3. `handleMessage` ([src/agent.ts](../src/agent.ts)) appends to `messages`, builds the per-turn context block (current local time, timezone, location), and calls the selected provider.
...
- **Circuit breaker.** `enforceStartupBackoff` ([src/lib/circuit-breaker.ts](../src/lib/circuit-breaker.ts)) sleeps before booting if recent restarts have been suspiciously frequent. Stops a crash loop from burning API quota.
...
- **Cost tracker.** [src/lib/cost-tracker.ts](../src/lib/cost-tracker.ts) sums `SDKResultSuccess.total_cost_usd` per day. New turns refuse if today's spend exceeds `daily_usd_budget` — but only on a metered Anthropic API key; Claude Pro/Max OAuth bypasses the check (no per-token billing).
- **Conversation archive.** [src/lib/conversation-archive.ts](../src/lib/conversation-archive.ts) keeps a JSONL transcript per thread under `data/conversations/`.
- **Backups.** [src/lib/backup.ts](../src/lib/backup.ts) snapshots `marsclaw.db`, `MEMORY.md`, and `data/whatsapp-auth/` on a schedule.
- **Health server.** [src/lib/health-server.ts](../src/lib/health-server.ts) exposes a small HTTP endpoint for liveness probes.
